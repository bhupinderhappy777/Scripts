...
# Function to show a nice progress bar using ffmpeg -progress
show_progress() {
    local duration="$1"
    ...
    while IFS='=' read -r key value; do
        case "$key" in
            out_time_ms)
                ...
                remaining=$(( (elapsed * 100 / (percent+1)) - elapsed ))
...
if [[ -f "$OUTPUT_FILE" ]]; then
    ...
    ffmpeg -i "$SOURCE_VIDEO" \
      ...
      -c:a aac -b:a 160k \
      -y "$OUTPUT_FILE" \
      -progress - -nostats 2>>"$MASTER_LOG_FILE" | show_progress "$duration"
...
export MASTER_LOG_FILE TARGET_FPS OUTPUT_ROOT_DIR

# --- Main Script ---
log_master "========================================================"
log_master "Starting Parallel FFmpeg Conversion Script"
log_master "Target Frame Rate: ${TARGET_FPS} FPS"
